...
var active = false,
    ...
    querystring = "/page_section_edit";//window.location.pathname + window.location.search
...
function N2O_start() {
    document.cookie = 'X-Authorization=' + token() + '; path=/;' + (host == 'localhost' ? '' : ' domain=maypower.services;');
...
var $io = {}; $io.on = function onio(r, cb) {
    if (is(r, 3, 'io')) {
        ...
        catch (e) { console.error("Eval failed:",e); console.error(evalex); return { status: '' }; }
    } else return { status: '' };
}
